...
const findClosestIntersection = (w1, w2) => {
  let closest = Infinity;
  let leastRes = Infinity;
  for (const p1 of w1.keys()) {
    if (! w2.has(p1)) {
      continue;
    }

    // an intersection!
    const [x, y] = p1.split(',').map(i => parseInt(i));
    const dist = Math.abs(x) + Math.abs(y);
    closest = dist < closest ? dist : closest;

    const resistance = w1.get(p1) + w2.get(p1);
    leastRes = resistance < leastRes ? resistance : leastRes;
  }

  return [closest, leastRes];
};

// w1 = parseLine('R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51');
// w2 = parseLine('U98,R91,D20,R16,D67,R40,U7,R15,U6,R7');
// console.log(w1);
// console.log(w2);

const s1 = parseLine(w1);
const s2 = parseLine(w2);

console.log(findClosestIntersection(s1, s2));
